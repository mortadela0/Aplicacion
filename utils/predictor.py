...
def _prep_X(df_tmp, target_col, X_cols):
    ...
    cols_nuevas = set(X.columns)
    cols_train  = set(X_cols)
    faltantes   = cols_train - cols_nuevas
    extras      = cols_nuevas - cols_train
    if faltantes:
        print(f"\n  {Fore.YELLOW}[WARN] Columnas del training ausentes en CSV nuevo (se rellenan con 0):")
        for c in sorted(faltantes): print(f"  {Fore.YELLOW}    - {c}")
    if extras:
        print(f"  {Fore.YELLOW}[WARN] Columnas extra en CSV nuevo (se ignoran):")
        for c in sorted(extras): print(f"  {Fore.YELLOW}    + {c}")
