    ...
    while True:
        ...
        if opcion == "1":
            ...
            if df is None or target_col is None:
                ...
                scores = sel.gower(df, target_col)
            ...
            if df is None or target_col is None:
                ...
                # Intentar Weka solo si está disponible y no se ejecutó antes
                if resultados_weka is None and weka_disponible():
                    resp = input(f"  {Fore.CYAN}¿Ejecutar Weka? (S/n): {Fore.GREEN}").strip().lower()
                    if resp != "n":
                        try:
                            resultados_weka = m_weka.ejecutar(df, target_col)
                        except Exception as e:
                            print(f"\n  {Fore.YELLOW}[WARN] Weka falló ({e}).")
                            print(f"  {Fore.YELLOW}  Continuando comparación sin Weka...")
                            resultados_weka = {"disponible": False}
